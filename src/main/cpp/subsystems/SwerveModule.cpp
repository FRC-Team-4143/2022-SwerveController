...
frc::SwerveModuleState SwerveModule::GetState() {
  return {units::meters_per_second_t{m_driveEncoder.GetRate()},
          ...
          frc::Rotation2d(units::radian_t(m_turningMotor.GetSelectedSensorPosition()/4096*2*wpi::numbers::pi))};
}
...
void SwerveModule::SetDesiredState(
  ...
  const auto state = frc::SwerveModuleState::Optimize(
      referenceState, units::radian_t(m_turningMotor.GetSelectedSensorPosition()/4096*2*wpi::numbers::pi));
  ...
  auto turnOutput = m_turningPIDController.Calculate(
      units::radian_t(m_turningMotor.GetSelectedSensorPosition()/4096*2*wpi::numbers::pi), state.angle.Radians());
